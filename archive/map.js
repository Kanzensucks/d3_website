...
// Each panel is a "hospital pictograph": the state's silhouette sits at the
// bottom and a stack of hospital icons (1 icon = PICTO_UNIT per 100,000) rises
// above it — height encodes the per-capita rate, so NT towers.
const PICTO_UNIT = 25;
...
function renderCompareStrip() {
  ...
  codes.forEach(code => {
    ...
    panel.append("svg").attr("class", "cmp-col").attr("preserveAspectRatio", "xMidYMax meet");
...
// GenAI-assisted (Claude): draw one pictograph column — state shape anchored at
// the bottom, hospital icons stacked above it, top icon clipped to the remainder.
function drawHospitalColumn(svgSel, code) {
  const VB_W = 84, VB_H = 312, shapeW = 66, shapeH = 46;
  const iconW = 26, iconH = 16, gap = 2;
  svgSel.attr("viewBox", `0 0 ${VB_W} ${VB_H}`).selectAll("*").remove();

  const row = vizState.byYear.get(vizState.currentYear).get(code);
  const r = row.cases_per_100k, col = fillFor(code);

  // state silhouette at the bottom
  const f = vizState.geo.features.find(x => x.properties.code === code);
  const proj = d3.geoIdentity().reflectY(true).fitSize([shapeW, shapeH], f);
  svgSel.append("path").attr("d", d3.geoPath(proj)(f))
    .attr("transform", `translate(${(VB_W - shapeW) / 2},${VB_H - shapeH})`)
    .attr("fill", col).attr("opacity", 0.32).attr("stroke", col).attr("stroke-width", 0.8);

  // hospital icons rising above the shape
  const units = r / PICTO_UNIT, full = Math.floor(units), frac = units - full;
  const ix = (VB_W - iconW) / 2, baseline = VB_H - shapeH - 3;
  const drawIcon = (parent, y) => {
    parent.append("rect").attr("x", ix).attr("y", y).attr("width", iconW).attr("height", iconH)
      .attr("rx", 2.5).attr("fill", col).attr("stroke", "#fff");
    parent.append("rect").attr("x", ix + iconW / 2 - 1.5).attr("y", y + iconH / 2 - 4).attr("width", 3).attr("height", 8).attr("fill", "#fff");
    parent.append("rect").attr("x", ix + iconW / 2 - 4).attr("y", y + iconH / 2 - 1.5).attr("width", 8).attr("height", 3).attr("fill", "#fff");
  };
  for (let k = 0; k < full; k++) drawIcon(svgSel, baseline - iconH - k * (iconH + gap));
  if (frac > 0.02) {
    const k = full, y = baseline - iconH - k * (iconH + gap), show = frac * iconH, id = "picto-" + code;
    const cp = svgSel.append("clipPath").attr("id", id);
    cp.append("rect").attr("x", ix).attr("y", y + iconH - show).attr("width", iconW).attr("height", show);
    drawIcon(svgSel.append("g").attr("clip-path", `url(#${id})`), y);
  }
}
...
function refreshCompareStripValues() {
  ...
  d3.selectAll("#compare-holder .cmp-panel").each(function () {
    ...
    drawHospitalColumn(sel.select("svg.cmp-col"), code);
